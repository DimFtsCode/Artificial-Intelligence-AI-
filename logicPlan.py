...
def sentence1() -> Expr:
    ...
    b = logic.Expr('B')
    c = logic.Expr('C')
    a1 = logic.disjoin(a,b)
    bnot = ~b;
    bc = bnot | c;
    anot = ~a;
    a2 = anot % bc

    a3 = logic.disjoin(anot,bnot,c)
    return logic.conjoin(a1,a2,a3)
...
def sentence2() -> Expr:
    ...
    a3 = ~bnotc >> a
    a4 = notd >> c
    return logic.conjoin(a1,a2,a3,a4)
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    a = logic.PropSymbolExpr("PacmanAlive_0")
    b = logic.PropSymbolExpr("PacmanAlive_1")
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    a = premise
    b = ~conclusion
    ab = logic.conjoin(a,b)
    return not findModel(ab)
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    notinv = ~inverse_statement
    return pl_true(notinv , assignments)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for expr in literals:
        for exprleft in literals:
            if expr != exprleft:
                dj = logic.disjoin(~expr , ~exprleft)
                cj.append(dj)
        
    return logic.conjoin(cj)   
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    pc = logic.disjoin(possible_causes)
    prev_states = atLeastOne(pc)
    print(prev_states)
    print(current)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x,y in all_coords:  
        NW = (PropSymbolExpr(wall_str,x,y) >> ~PropSymbolExpr(pacman_str,x,y,time = t))
        walls.append(NW)
    wallsF = conjoin(walls)
    
    
    nout_wall = [] 
    for x,y in non_outer_wall_coords:
        nout_wall.append(PropSymbolExpr(pacman_str,x,y,time = t))
    OneOfNout = exactlyOne(nout_wall)
    
    
    directions = []
    for direction in DIRECTIONS:
        directions.append(PropSymbolExpr(direction ,time = t))
    OneOfDirections = exactlyOne(directions)
    
    pacphysics_sentences = conjoin(wallsF, OneOfNout,OneOfDirections)
    
    if(sensorModel):
        a = sensorModel(t , non_outer_wall_coords)
        pacphysics_sentences = conjoin(pacphysics_sentences,a)
        
    if(successorAxioms and t!=0):
        b =  successorAxioms(t, walls_grid,non_outer_wall_coords)
        pacphysics_sentences = conjoin(pacphysics_sentences, b)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    a = pacphysicsAxioms(0, all_coords, non_outer_wall_coords, walls_grid, None, None)
    KB.append(a)
    KB.append(pacphysicsAxioms(1, all_coords, non_outer_wall_coords, walls_grid,None, allLegalSuccessorAxioms))
    KB = logic.conjoin(KB)
    pacnow = PropSymbolExpr(pacman_str,x1,y1,time = 1)
    model1 = findModel(conjoin(KB,pacnow))
    model2 = findModel(conjoin(KB, ~pacnow))
    tup = (model1,model2)
    return tup
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(0,50):
        ...
        for x,y in non_wall_coords:
            nout.append(PropSymbolExpr(pacman_str,x,y,time = t))
            succ.append(pacmanSuccessorAxiomSingle(x,y,t+1,walls_grid))
        ...
        for action in actions:
            actions1.append(PropSymbolExpr(action , time = t))
            
        
        KB.append(exactlyOne(actions1))
        
        KB.append(exactlyOne(nout))
        
        KB = KB + succ
        goalstate = PropSymbolExpr(pacman_str,xg,yg , time = t)
        KBL = conjoin(KB)
        KBL = conjoin(KBL , goalstate)
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(0,50):
        ...
        for x,y in non_wall_coords:
            nout.append(PropSymbolExpr(pacman_str,x,y,time = t))
            succ.append(pacmanSuccessorAxiomSingle(x,y,t+1,walls))
            noutf.append(PropSymbolExpr(food_str,x,y,time = t))
            
        goalstate = ~disjoin(noutf)
        ...
        KB.append(succ)
        for x,y in non_wall_coords:
            A = ~PropSymbolExpr(pacman_str,x,y,time = t)
            B = PropSymbolExpr(food_str,x,y,time =t)
            B1 = PropSymbolExpr(food_str,x,y,time = t+1) 
            conc = B1 % conjoin(A , B)
            KB.append(conc)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x,y in non_outer_wall_coords:
            premise = conjoin(KB)
            conclusion = PropSymbolExpr(pacman_str,x,y,time = t)
            if entails(premise,conclusion):
                ...
                if entails(premise,~conclusion):
                    ...
                    print("Not entails with conclusion and not conclusion")
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x,y in non_outer_wall_coords:
            premise = conjoin(KB)
            conclusion = PropSymbolExpr(wall_str,x,y)
            if entails(premise,conclusion):
                known_map[x][y] = 1
                KB.append(PropSymbolExpr(wall_str,x,y))
            else:
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x,y in non_outer_wall_coords:
            premise1 = conjoin(KB)
            conclusion1 = PropSymbolExpr(wall_str,x,y)
            if entails(premise1,conclusion1):
                known_map[x][y] = 1
                KB.append(PropSymbolExpr(wall_str,x,y))
            else:
                if entails(premise1,~conclusion1):
            ...
            premise2 = conjoin(KB)
            conclusion2 = PropSymbolExpr(pacman_str,x,y,time = t)
            if entails(premise2,conclusion2):
                KB.append(PropSymbolExpr(pacman_str,x,y,time = t))
                possible_locations.append((x,y))
            else:
                if entails(premise2,~conclusion2):
